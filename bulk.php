		...
		if ( 'true' === $resume ) {
			...
			if ( ! empty( $_REQUEST['ids'] ) && ( preg_match( '/^[\d,]+$/', $_REQUEST['ids'] ) || is_numeric( $_REQUEST['ids'] ) ) ) {
				echo '<p class="ewww-media-info ewww-bulk-info">' .
					...
					sprintf( esc_html( _n( '%1$s item in the Media Library has been selected with up to %2$d image files per upload.', '%1$s items in the Media Library have been selected with up to %2$d image files per upload.', $fullsize_count, 'ewww-image-optimizer-cloud' ) ), number_format_i18n( $fullsize_count ), $resize_count ) .
...
function ewww_image_optimizer_bulk_reset_form_output() {
	?>
		<p class="ewww-media-info ewww-bulk-info" style="margin-top:3em;"><?php esc_html_e( 'Would like to start over and rescan for images?', 'ewww-image-optimizer-cloud' ); ?></p>
		<form class="ewww-bulk-form" method="post" action="">
			...
			<button id="ewww-bulk-reset" type="submit" class="button-secondary action"><?php esc_html_e( 'Reset Scanner', 'ewww-image-optimizer-cloud' ); ?></button>
		</form>
	<?php
}
