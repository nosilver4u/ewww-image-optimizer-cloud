...
function ewww_image_optimizer_aux_images_table() {
	...
	if ( ! empty( $search ) ) {
		...
		if ( $search_count < 50 ) {
			...
			echo "<span id='ewww-search-result' style='display:none'>" . sprintf( esc_html__( '%d items found', 'ewww-image-optimizer-cloud' ), count( $already_optimized ) ) . "</span>\n";
		} else {
			/* translators: 1: number of image records displayed, 2: number of total records found */
			echo "<span id='ewww-search-result' style='display:none'>" . sprintf( esc_html__( '%1$d items displayed of %2$d records found', 'ewww-image-optimizer-cloud' ), count( $already_optimized ), $search_count ) . "</span>\n";
		}
		...
		/* translators: %d: number of image records found */
		echo "<span id='ewww-search-result' style='display:none'>" . sprintf( esc_html__( '%d items displayed', 'ewww-image-optimizer-cloud' ), count( $already_optimized ) ) . "</span>\n";
		echo "<span id='ewww-search-count-raw' style='display:none'>" . count( $already_optimized ) . "</span>\n";
	}
	$upload_info = wp_upload_dir();
	$upload_path = $upload_info['basedir'];
	echo '<table class="wp-list-table widefat media" cellspacing="0"><thead><tr><th>&nbsp;</th><th>' . esc_html__( 'Filename', 'ewww-image-optimizer-cloud' ) . '</th><th>' . esc_html__( 'Image Type', 'ewww-image-optimizer-cloud' ) . '</th><th>' . esc_html__( 'Image Optimizer', 'ewww-image-optimizer-cloud' ) . '</th></tr></thead>';
	$alternate = true;
